...
type View = 'hub' | 'mistakes' | 'practice' | 'practice-result' | 'cards' | 'cards-done'
...
export function ReviewPage() {
  ...
  const allDueCards = useMemo(
    () => course.lessons.flatMap((lesson) => lesson.cards).filter((card) => !progress.masteredCards.includes(card.id)),
    [course.lessons, progress.masteredCards],
  )
    ...
    return (
      <div className="quiz-active">
        ...
        <MistakeReviewCard
          ...
          footerHint={isLast ? 'Можно сразу потренировать эти вопросы' : 'Разберите каждый ошибочный ответ'}
          footerAction={(
            <div className="mistake-footer-actions">
              ...
              <button className="primary-button" type="button" onClick={nextMistake}>
                {isLast ? 'К повторению' : 'Следующая ошибка'} <ArrowRight size={18} />
              </button>
            </div>
          )}
        />
      ...
      <div className="review-page">
        <header className="review-header">
          <div>
            ...
            <p>До {dailyCardLimit} карточек за одну сессию — удобный ритм без перегрузки.</p>
          </div>
  ...
  const hasAnything = mistakes.length > 0 || allDueCards.length > 0
